use crate::axync::{
  Receiver, RecvError, Sender, WaitGroup, bounded, select, stop_channel, unbounded,
};
  ...
  S: BuildHasher + Clone + 'static + Send + Sync,
{
    ...
    <RT as RuntimeLite>::Interval: Send,
  {
    ...
    let this = AsyncCache {
      ...
      close_lock: Arc::new(AsyncMutex::new(())),
      coster,
      metrics,
  ...
  KH: KeyBuilder<Key = K>,
{
  ...
  pub(crate) close_lock: Arc<AsyncMutex<()>>,
  ...
  S: BuildHasher + Clone + 'static + Send,
{
  ...
  #[cfg_attr(not(tarpaulin), inline(always))]
  async fn clear_inner(&self) -> Result<(), CacheError> {
  ...
  ///
  /// Concurrent calls are serialized: callers that arrive while another task
  /// is performing the shutdown await the in-flight close, and every caller
  /// that receives `Ok(())` is guaranteed to observe the fully cleared state
  /// (empty store, zeroed metrics, stopped processor).
  #[cfg_attr(not(tarpaulin), inline(always))]
