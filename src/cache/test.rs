...
mod sync_test {
  ...
  fn test_cache_insert_after_close() {
    let c =
      new_test_cache::<u64, u64, TransparentKeyBuilder<u64>>(TransparentKeyBuilder::default());
    let _ = c.close();
    assert!(!c.insert(1, 1, 1));
  }
  ...
  #[test]
  fn test_sync_get_mut_after_close() {
    let c =
      new_test_cache::<u64, u64, TransparentKeyBuilder<u64>>(TransparentKeyBuilder::default());
    assert!(c.insert(1, 1, 1));
  ...
  #[test]
  fn test_sync_close_actually_clears_state() {
    // Regression: close() used to set is_closed before calling clear(),
    // which made clear() early-return and leave state behind.
    let c = Cache::builder(100, 10)
      .set_key_builder(TransparentKeyBuilder::default())
      .set_metrics(true)
      .set_ignore_internal_cost(true)
    ...
    // Store, policy, and metrics must all be cleared by close.
    assert_eq!(c.len(), 0);
    assert!(c.is_empty());
    assert_eq!(c.metrics.get_keys_added(), Some(0));
  }
  ...
  #[test]
  fn test_sync_concurrent_close() {
    // All concurrent close() calls return Ok; only one performs the work.
    let c: Cache<u64, u64, TransparentKeyBuilder<u64>> = Cache::builder(100, 10)
      .set_key_builder(TransparentKeyBuilder::default())
      .set_ignore_internal_cost(true)
      .finalize()
  ...
  #[test]
  fn test_sync_close_is_completion_barrier() {
    // Regression: losers of the close() race used to return Ok before the
    // winner finished clear_inner/stop/policy.close, so they could observe
    // non-empty state. After the fix every returning caller must see the
    // fully shut-down cache.
    use std::sync::Barrier;

    for _ in 0..16 {
      let c: Cache<u64, u64, TransparentKeyBuilder<u64>> = Cache::builder(10_000, 10_000)
        .set_key_builder(TransparentKeyBuilder::default())
        .set_metrics(true)
        .set_ignore_internal_cost(true)
        .finalize()
      ...
      let handles: Vec<_> = (0..8)
        .map(|_| {
          ...
          spawn(move || {
            b.wait();
            tc.close().unwrap();
            // Every caller that returned Ok must observe the shut-down state.
            assert_eq!(tc.len(), 0);
            assert!(tc.is_empty());
            assert_eq!(tc.metrics.get_keys_added(), Some(0));
          })
        })
        .collect();
      for h in handles {
...
mod async_test {
  ...
  #[tokio::test]
  async fn test_async_get_mut_after_close() {
    let c =
      new_test_cache::<u64, u64, TransparentKeyBuilder<u64>>(TransparentKeyBuilder::default())
        .await;
    assert!(c.insert(1, 1, 1).await);
    ...
    let _ = c.close().await;
    assert!(c.get_mut(&1).await.is_none());
  }
  ...
  #[tokio::test]
  async fn test_async_close_actually_clears_state() {
    // Regression: async close raced on is_closed and previously could
    // skip the clear body when paired with the sync close bug pattern.
    let c = AsyncCacheBuilder::<u64, u64>::new(100, 10)
      .set_key_builder(TransparentKeyBuilder::default())
      .set_metrics(true)
      .set_ignore_internal_cost(true)
      .finalize::<TokioRuntime>()
  ...
  #[tokio::test]
  async fn test_async_concurrent_close() {
    let c = AsyncCacheBuilder::<u64, u64>::new(100, 10)
      .set_key_builder(TransparentKeyBuilder::default())
      .set_ignore_internal_cost(true)
      .finalize::<TokioRuntime>()
    ...
    let handles: Vec<_> = (0..8)
      .map(|_| {
        let tc = c.clone();
        tokio::spawn(async move { tc.close().await })
      })
      .collect();
    for h in handles {
  ...
  #[tokio::test]
  async fn test_async_close_is_completion_barrier() {
    // Regression: losers of the async close() race used to return Ok before
    // the winner finished clear/stop/policy.close. After the fix, every
    // returning caller must observe the fully shut-down cache.
    for _ in 0..16 {
      let c = AsyncCacheBuilder::<u64, u64>::new(10_000, 10_000)
        .set_key_builder(TransparentKeyBuilder::default())
        .set_metrics(true)
        .set_ignore_internal_cost(true)
        .finalize::<TokioRuntime>()
